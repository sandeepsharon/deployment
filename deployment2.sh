...
if [ $count -ne 0 ]; then
 ...
 for i in $list;
  do
   if [[ "$i" =~ ^($war1|$war2|$war3|$war4)$ ]]; then
    ...
    mv $webapps/$i $backup_location/$current_date/${i}_$time > /dev/null 2>&1
    if [ $i == $war2 ]; then
     war_cut
     ((ping_flag++))
    fi
    ...
    cp -R $files/$i $backup_location/$current_date/${i}_$time > /dev/null 2>&1
 ...
 sleep 1
  ...
  if [ $flag -gt 0 ]; then
   ...
   ping
   echo "Scanning war files"
  fi
  if [ $ping_flag -gt 0 ]; then
   echo "Deleting files after war explosion"
   systemctl stop tomcat > /dev/null 2>&1
   sleep 5
   tomcat_shutdown
   sleep 5
